<?php
    class estudianteModel {
        ...
        public function __construct() {
            require_once($_SERVER['DOCUMENT_ROOT'].'/tallerphp18/routes.php');
        ...
        public function listar() {
            $sql = 'SELECT e.idEstudiante,e.nombre,e.apellido,e.idCurso, c.nombre as cursos FROM estudiantes e JOIN cursos c ON e.idCurso = c.idCurso ORDER BY idEstudiante DESC';
            $statement = $this->PDO->prepare($sql);
        ...
        public function buscar($idEstudiante) {
            $sql = 'SELECT e.idEstudiante,e.nombre,e.apellido,e.idCurso, c.nombre as cursos FROM estudiantes e JOIN cursos c ON e.idCurso = c.idCurso WHERE idEstudiante = :idEstudiante';
            $statement = $this->PDO->prepare($sql);
            $statement->bindParam(':idEstudiante', $idEstudiante);
